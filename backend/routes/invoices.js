 ...
 */
function validateInvoiceInput({ company, associateId, title, amount }) {
  if (!company?.trim()) return 'Company name required';
  if (!associateId) return 'Associate ID required';
  if (!title?.trim()) return 'Invoice title required';
  if (!amount || amount <= 0) return 'Valid amount required';
  return null;
}
