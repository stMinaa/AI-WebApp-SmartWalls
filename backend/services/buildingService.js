...
async function createBuilding(data) {
  ...
  if (!name || !name.trim()) throw { status: 400, message: 'Building name required' };
  if (!address || !address.trim()) throw { status: 400, message: 'Address required' };

  const existing = await Building.findOne({ address: address.trim() });
  if (existing) throw { status: 400, message: 'Building at this address already exists' };
...
async function bulkCreateApartments(buildingId, data) {
  ...
  // Input validation
  if (!startUnit || !count) throw { status: 400, message: 'startUnit and count required' };

  const startNum = Number(startUnit);
  const countNum = Number(count);
  if (isNaN(startNum) || isNaN(countNum) || startNum < 1 || countNum < 1) {
    throw { status: 400, message: 'Invalid startUnit or count value' };
  }
