...
describe('Issue Triage - PATCH /api/issues/:issueId/triage', () => {
  ...
  beforeEach(async () => {
    ...
    const associate = await signupUser({
      username: 'triageassoc',
      email: 'triageassoc@test.com',
      password: 'Test123!',
      firstName: 'Triage',
      lastName: 'Associate',
      role: 'associate'
    ...
    const tenant = await signupUser({
      username: 'triagetenant',
      email: 'triagetenant@test.com',
      password: 'Test123!',
      firstName: 'Triage',
      lastName: 'Tenant',
      role: 'tenant'
    ...
    const buildingId = await createBuilding(directorToken, {
      name: 'Triage Zgrada',
      address: 'Triage Ulica 1'
    });
    ...
    issueId = await createIssue(tenantToken, {
      title: 'Curi voda u kupatilu',
      description: 'Slavina curi vec tri dana',
      priority: 'high'
    });
  });
  ...
  test('should return 401 without token', async () => {
    const res = await request(app).patch(`/api/issues/${issueId}/triage`);
    assertError(res, 401);
  });
